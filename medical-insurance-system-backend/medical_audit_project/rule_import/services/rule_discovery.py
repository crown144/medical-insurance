...
def build_prompt(headers, rows):
   ...
   - 项目A的项目内涵包含项目B” → 项目A和项目B不能同时收取
    ...
    【规则类型】
    ========================
    1. DRUG_RESTRICTION（药品超限定）：限XX疾病/适应症/二线治疗/年龄/科室
    2. FREQUENCY_LIMIT（超频次收费）：单日上限X次/每日不超过X次
    3. DUPLICATE_CHARGE（重复收费）：项目A包含项目B/不得同时收费/不得重复收费
    4. PRICE_LIMIT（超标准收费）：收费单位说明/加收减收说明
    5. CONSUMABLE_RESTRICTION（耗材超限定）：限特定手术/病种/适应症
    6. OVER_EXAMINATION（过度检查）：不宜重复检查/限必要时开展/开展周期限制
    7. OTHER：无法归入以上类别但能形成可执行审核逻辑的规则
