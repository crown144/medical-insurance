...
async function handleDelete(row: RuleRow) {
  await ElMessageBox.confirm(`确定删除规则【${row.drug_name}】？`, '删除确认', {
    type: 'warning',
...
async function save() {
  if (!form.description.trim()) return ElMessage.warning('请填写【原始描述】');
...
async function bulkDelete() {
  ...
  await ElMessageBox.confirm(
    `确定批量删除 ${selectedRows.value.length} 条规则？`,
    '批量删除',
    {
      type: 'warning',
      confirmButtonText: '删除',
...
async function bulkSetEnabled(val: boolean) {
  ...
  ElMessage.success(val ? '已批量启用' : '已批量停用');
}
...
async function bulkExportJson() {
  ...
  try {
    ...
    ElMessage.success('已复制到剪贴板（JSON）');
  } catch {
    ElMessage.warning('复制失败：浏览器权限限制，可在控制台获取 selectedRows');
...
function openHtml(row: RuleRow) {
  htmlTitle.value = `来源HTML - ${row.drug_name}`;
  htmlContent.value =
    row.sourceHtml?.trim() ||
    `<p style="color:#999">暂无 sourceHTML</p>`;
  ...
  <div class="page-card">
    ...
    <div class="header">
      ...
      <div class="header-actions">
        <el-select v-model="ruleTypeSelect" style="width: 160px">
          <el-option label="超限定用药" value="超限定用药" />
        ...
        <el-input
          ...
          clearable
          style="width: 320px"
          placeholder="搜索：规则/原始描述/来源/逻辑表达式"
        />
    ...
    <div v-if="selectedRows.length > 0" class="bulk-bar">
      ...
      <div class="bulk-right">
        <el-button @click="bulkExportJson">批量导出JSON</el-button>
        <el-button @click="bulkSetEnabled(true)">批量启用</el-button>
        <el-button @click="bulkSetEnabled(false)">批量停用</el-button>
    ...
    >
      ...
      <!-- ② 规则ID -->
      <el-table-column prop="ruleId" label="规则ID" width="120" sortable />

      <!-- ③ 规则名称 -->
      <el-table-column prop="drug_name" label="规则名称" min-width="160" />
      ...
      <!-- ⑥ 规则来源 -->
      <el-table-column label="规则来源" min-width="260">
        <template #default="scope">
          <div class="source-cell">
            <div class="source-text" :title="scope.row.sourceLocate">
              {{ scope.row.sourceLocate }}
            </div>
            <el-button link type="primary" @click="openHtml(scope.row)">
              HTML
            </el-button>
          </div>
        </template>
    ...
    >
      <el-form label-width="140px">
        <el-form-item label="规则（自动提取）">
          <el-input
            v-model="form.drug_name"
            placeholder="可为空：会从原始描述自动提取（如 艾普拉唑 / 胃肠减压）"
          />
        </el-form-item>

        <el-form-item label="规则类型（下拉）">
        ...
        <el-form-item label="原始描述（必填）">
          <el-input v-model="form.description" type="textarea" :rows="3" />
        </el-form-item>

        <el-form-item label="来源（定位）">
          <el-input
            v-model="form.sourceLocate"
            placeholder="未知先空；你可以先填规则来源文本"
          />
        </el-form-item>

        <el-form-item label="来源HTML（可选）">
          <el-input
    ...
    >
      ...
      >
        ...
        <el-descriptions-item label="规则类型">
          {{ currentDetail.ruleType }}
        </el-descriptions-item>
        <el-descriptions-item label="规则状态">
          <el-tag :type="currentDetail.enabled ? 'success' : 'danger'">
            {{ currentDetail.enabled ? '启' : '停' }}
          </el-tag>
        </el-descriptions-item>
        <el-descriptions-item label="来源定位" :span="2">
          {{ currentDetail.sourceLocate || '暂无' }}
        </el-descriptions-item>
        <el-descriptions-item label="来源HTML" :span="2">
        ...
        <el-descriptions-item label="规则执行函数" :span="2">
          <div class="code-scroll-box">
            <pre>{{ currentDetail.ruleCode || '暂无代码' }}</pre>
          </div>
        </el-descriptions-item>
      </el-descriptions>
    </el-dialog>
    ...
    >
      ...
      <template #footer>
        <el-button @click="htmlVisible = false">关闭</el-button>
      </template>
    </el-dialog>
  </div>
</template>
